...
load("MS__RgSet_Methylumi_Pheno_PFC142_STG144.Rdata")
###################################
#######MS.PFC
setwd("/mnt/data1/Ehsan/meta_CrossCortical/MS/QC")
###### QC.1#######
rgSet<-rgSet.MS.PFC
Methylumiset<-MS.PFC
pheno<-pheno.MS.PFC
...
#Samples removed:  
#   858 sites were removed as beadcount <3 in 5 % of samples 
#   1481 sites having 1 % of samples with a detection p-value greater than 0.05 were removed 
...
save(betas,pheno, file="MS.PFC.141.beta.pheno.Rdata")
#TRUE
...
save(dat.reg,pheno, file="MS.PFC.141.pheno.REGRESSED.Rdata")
...
save(sva, file="sva.regressed.ms.pfc.Rdata")

svs<-sva$sv
pheno.MS<-cbind(pheno, svs)

setwd("/mnt/data1/Ehsan/meta_CrossCortical/pheno/")
write.csv(pheno.MS,file="pheno.MS.PFC.141.csv")
########################################################################################################################
########################################################################################################################
########################################################################################################################
########################################################################################################################
...
###################################
#######MS.STG
setwd("/mnt/data1/Ehsan/meta_CrossCortical/MS/QC/STG")
###### QC.1#######
rgSet<-rgSet.MS.STG
Methylumiset<-MS.STG
pheno<-pheno.MS.STG
...
#0 samples having 5 % of sites with a detection p-value greater than 0.05 were removed 
#Samples removed:  
#   317 sites were removed as beadcount <3 in 5 % of samples 
#   1291 sites having 1 % of samples with a detection p-value greater than 0.05 were removed 
dat.dasen<-dasen(dat.pf)
betas.dasen<-betas(dat.dasen)
pheno<-pheno[-which(colnames(Methylumiset)%in%bad),]
identical(colnames(betas.dasen), as.character(pheno$X))
...
colnames(betas)<-as.character(pheno$X)
setwd("/mnt/data1/Ehsan/meta_CrossCortical/NORMALISED_BETA")
save(betas,pheno, file="MS.STG.143.beta.pheno.Rdata")
#TRUE
...
setwd("/mnt/data1/Ehsan/meta_CrossCortical/REGRESSED_BETA")
save(dat.reg,pheno, file="MS.STG.143.pheno.REGRESSED.Rdata")
...
sva<- sva::sva(dat = as.matrix(mydf), mod = mod, mod0 = mod0)
setwd("/mnt/data1/Ehsan/meta_CrossCortical/SVA/")
save(sva, file="sva.regressed.ms.stg.Rdata")
load("sva.regressed.ms.stg.Rdata")
svs<-data.frame(sva$sv)

pheno.mtg<-cbind(pheno, svs)

setwd("/mnt/data1/Ehsan/meta_CrossCortical/pheno/")
write.csv(pheno.mtg,file="pheno.MS.STG.143.csv")
