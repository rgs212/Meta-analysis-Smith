...
load("AZ2_RgSet_Methylumi_Pheno_95.Rdata")
###################################
#######AZ2
setwd("/mnt/data1/Ehsan/meta_CrossCortical/AZ2/QC")
###### QC.1#######
rgSet<-rgsetAZ2
Methylumiset<-AZ2
pheno<-pheno.AZ2
...
identical(colnames(Methylumiset), as.character(pheno$Sentrix))
#[1] TRUE
...
Position<-as.factor(substr(pheno$Sentrix,12,18))
...
Chip<-as.factor(substr(pheno$Sentrix,1,11))
...
dev.off()
BAD.samples.sexMissmatch<-as.character(pheno[which(predSex!=pheno$Gender),]$Sentrix)
############################################################################
############################################################################
############################################################################
...
#Samples removed:  
#   155 sites were removed as beadcount <3 in 5 % of samples 
#   3279 sites having 1 % of samples with a detection p-value greater than 0.05 were removed 
dat.dasen<-dasen(dat.pf)
betas.dasen<-betas(dat.dasen)
pheno<-pheno[-which(colnames(Methylumiset)%in%bad),]
identical(colnames(betas.dasen), as.character(pheno$Sentrix))
#[1] TRUE
...
#[1] 424958    141
betas<-data.frame(d2)
identical(substr(colnames(betas),2,18),as.character(pheno$Sentrix))
colnames(betas)<-as.character(pheno$Sentrix)
setwd("/mnt/data1/Ehsan/meta_CrossCortical/NORMALISED_BETA")
save(betas,pheno, file="AZ2.MTG.88.beta.pheno.Rdata")
#TRUE
...
betas<-betas[,as.character(pheno$Sentrix)]


colData(rgsetAZ2)@listData$sentrixFull <- as.character(colData(rgsetAZ2)@listData$sentrixFull)
library(minfi)
...
cet.az2 <- estimateCellCounts(rgsetAZ2[,colnames(betas)], 
                              ...
                              meanPlot = TRUE, 
                              verbose = TRUE)
cellprop<-(cet.az2[,2]/(cet.az2[,2]+cet.az2[,1]))*100
pheno<-cbind(pheno, cellprop)
pheno$Braak<-pheno$Braakscore



dat.reg<- {              
  sex    	<- 	as.factor(pheno$Gender)
  age    	<- 	pheno$expired_age 
  cet		<- as.numeric(pheno$cellprop)
  t(parApply(cl, betas, 1, resid, age, sex, cet))
}
head(dat.reg[1:5,1:5])
identical(colnames(dat.reg), as.character(pheno$Sentrix))

setwd("/mnt/data1/Ehsan/meta_CrossCortical/REGRESSED_BETA")
save(dat.reg,pheno, file="AZ2.MTG.88.pheno.REGRESSED.Rdata")
...
save(sva, file="sva.regressed.AZ2.mtg.Rdata")


svs<-sva$sv
pheno.AZ1<-cbind(pheno, svs)

setwd("/mnt/data1/Ehsan/meta_CrossCortical/pheno/")
write.csv(pheno.AZ1,file="pheno.AZ1.88.csv")